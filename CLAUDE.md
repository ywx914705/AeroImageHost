...
- **`main.cc`** — Entry point. Initializes Config → AsyncLog → ConnectionPool → RedisClient → MinIOClient → AeroQueue (4 worker threads) → HttpServer, then waits for SIGINT/SIGTERM.
...
- **`storage/`** — Data access layer.
  ...
  - `MinIOClient` (singleton) — MinIO SDK wrapper. Handles put/get/delete/presign/compose (multipart merge).
...
- **Singletons everywhere**: `Config::instance()`, `ConnectionPool::getInstance()`, `RedisClient::instance()`, `MinIOClient::instance()`, `FileMetaDAO::instance()`, `AeroQueue::instance()`.
...
- **Two upload modes**: Direct upload (small files, server proxies to MinIO) and multipart/presigned upload (large files, client uploads directly to MinIO). The frontend auto-selects based on file size (5MB threshold).
...
`config.json` (gitignored, not committed) is required. A Docker-specific version exists at `config/config-docker.json`. Key sections: `http_port`, `max_file_size`, `mysql.*`, `redis.*`, `minio.*`, `smtp.*`, `log.*`.
...
System packages required (Ubuntu): `build-essential cmake libssl-dev libcurl4-openssl-dev libmysqlclient-dev libvips-dev pkg-config`. Also requires: `cpprestsdk` (built from source), `hiredis`, `miniocpp`, `curlpp`, `pugixml`, `iniparser`/`inih`.
